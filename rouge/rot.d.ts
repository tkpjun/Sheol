declare module ROT {
    ...
    interface ISpeedActor extends IActor {
        getSpeed(): number;
    }
    ...
    export class Text {
        ...
        static tokenize(str: string, maxWidth: number): Array<any>; //TODO
    }
...
declare module ROT.Scheduler {
    ...
    export class Action extends Scheduler {
        add(item: IActor, repeat: boolean, time?: number);
        clear();
        next();
        remove(item: any);
    ...
    export class Speed extends Scheduler {
        add(item: ISpeedActor, repeat: boolean);
        next();
    }
}
